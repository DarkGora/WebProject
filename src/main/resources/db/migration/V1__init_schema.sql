...
INSERT INTO employees (name, phone_number, email, telegram, resume, school, about, photo_path, position, department,
                       deleted, deleted_at)
VALUES ('Евушко Андрей', '+375336980732', 'anonim@mail.ru', '@ansgoo',
...
VALUES ((SELECT id FROM employees WHERE email = 'anonim@mail.ru'), 'JAVA'),
...
VALUES (2022, 2023, 'Университет им. Лермонтова', 'Бакалавр',
        (SELECT id FROM employees WHERE email = 'anonim@mail.ru')),
       (2022, 2023, 'Университет им. Пушкина', 'Магистр', (SELECT id FROM employees WHERE email = 'anonim@mail.ru')),

       (2023, 2024, 'Университет им. Лермонтова', 'Специалист',
        (SELECT id FROM employees WHERE email = 'test@mail.ru')),
...
-- Вставка отзывов
INSERT INTO reviews (employee_id, rating, comment, created_at)
VALUES ((SELECT id FROM employees WHERE email = 'anonim@mail.ru'), 5,
        'Отличный разработчик! Очень ответственный и квалифицированный.', CURRENT_TIMESTAMP),
       ((SELECT id FROM employees WHERE email = 'anonim@mail.ru'), 4,
        'Хорошо справляется с задачами, иногда нужно больше времени.', CURRENT_TIMESTAMP - INTERVAL '5 days'),

       ((SELECT id FROM employees WHERE email = 'test@mail.ru'), 4, 'Хороший тестировщик, находит сложные баги.',
        CURRENT_TIMESTAMP),
       ((SELECT id FROM employees WHERE email = 'test@mail.ru'), 3, 'Нужно улучшить документацию тестов.',
        CURRENT_TIMESTAMP - INTERVAL '10 days'),

       ((SELECT id FROM employees WHERE email = 'deleted@mail.ru'), 3, 'Старый отзыв на удаленного сотрудника.',
        CURRENT_TIMESTAMP - INTERVAL '30 days'),

       ((SELECT id FROM employees WHERE email = 'maria@mail.ru'), 5, 'Отличный full-stack разработчик!',
        CURRENT_TIMESTAMP),
...
ALTER TABLE employees
    ...
    ADD COLUMN IF NOT EXISTS theme               VARCHAR(50) NOT NULL DEFAULT 'dark',
    ADD COLUMN IF NOT EXISTS language            VARCHAR(10) NOT NULL DEFAULT 'ru',
    ADD COLUMN IF NOT EXISTS items_per_page      INTEGER     NOT NULL DEFAULT 10;

-- Создаем индекс для поиска по email (если его нет)
CREATE INDEX IF NOT EXISTS idx_employees_email ON employees (email);
